...
typedef CvResult (CV_API_CALL *cv_videoio_retrieve_mvs_cb_t)(int stream_idx, unsigned const char* data, int step, int width, int height, int cn, void* userdata);
...
typedef struct OpenCV_VideoIO_Plugin_API_preview
{
    ...
    CvResult (CV_API_CALL *Capture_retreive_mvs)(CvPluginCapture handle, int stream_idx, cv_videoio_retrieve_mvs_cb_t callback, void* userdata);
